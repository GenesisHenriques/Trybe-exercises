import pytest
from learning_tests.scenario1 import gerar_relatorio_simples
...
def test_data_mais_antiga_de_fabricacao(stock):
    report = gerar_relatorio_simples(stock)
    assert "Data de fabricação mais antiga: 2019-09-13" in report


def test_data_validade_mais_proxima(stock):
    report = gerar_relatorio_simples(stock)
    assert "Data de validade mais próxima: 2023-01-17" in report


def test_empresa_com_maior_quantidade_de_produtos(stock):
    report = gerar_relatorio_simples(stock)
    msg = "Empresa com maior quantidade de produtos estocados: sanofi-aventis U.S. LLC"
