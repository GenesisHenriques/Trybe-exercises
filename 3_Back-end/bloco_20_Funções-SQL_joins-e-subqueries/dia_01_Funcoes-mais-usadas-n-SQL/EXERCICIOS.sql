...
SELECT JOB_ID, COUNT(*) AS 'Quantidade' FROM hr.employees
...
-- 8. Utilizando a query anterior, faça as alterações para
-- que seja exibido somente a quantidade de dinheiro necessária
...
-- 9. Escreva uma query que exiba em ordem decrescente a média
-- salarial de todos os cargos, exceto das pessoas programadoras ( IT_PROG ).

SELECT JOB_ID, AVG(SALARY) AS 'MediaSalaria' FROM hr.employees
GROUP BY JOB_ID
HAVING JOB_ID <> 'IT_PROG'
ORDER BY MediaSalaria DESC;
...
SELECT
	DEPARTMENT_ID,
    AVG(SALARY) AS 'MediaSalarial',
    COUNT(*) AS 'QuantidadeDeFuncionarios'
FROM hr.employees
...
SELECT EMPLOYEE_ID, FIRST_NAME, DAY(HIRE_DATE) AS Dia FROM hr.employees;
...
SELECT EMPLOYEE_ID, FIRST_NAME, MONTH(HIRE_DATE) AS Mês FROM hr.employees;
...
SELECT
	FIRST_NAME,
    LAST_NAME,
    DATEDIFF(NOW(), HIRE_DATE) AS 'Tempo_de_trabalho'
FROM hr.employees;
